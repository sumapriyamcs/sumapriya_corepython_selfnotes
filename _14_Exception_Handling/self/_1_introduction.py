...
WildAnimal tiger = Tiger()  # Upcasting  5L Can <--- 2L Water
...
# # At a time we can handle one exception
try:
    x = int(input("Enter numerator value :"))
    y = int(input("Enter denominator value:"))
    print(x / y)  # 10/0 ===> ZeroDivisionError("division by zero")
    print("Hello world")
except ValueError as ve:  # XXX ValueError ve = ZeroDivisionError("division by zero")   XXXX
    print("EXC : Give only integer value")
except ZeroDivisionError as zde:  # ZeroDivisionError zde = ZeroDivisionError("division by zero")
    print("EXC : Please enter denominator value other than 0 ***", zde)
...
try:
    x = int(input(
        "Enter numerator value :"))  # At a time we can handle one exception only
    y = int(input("Enter denominator value:"))
    print(x / y)
    print("Hello world")
